...
class ChunkMetadata(BaseModel):
    ...
    breadcrumbs: List[str] = Field(
        default_factory=list, description="Percorso gerarchico"
    )
    ...
    page_range: Optional[List[int]] = Field(
        None, description="Range di pagine (per PDF)"
    )
    ...
    updated_at: datetime = Field(
        default_factory=datetime.now, description="Data aggiornamento"
    )

    # Relazioni gerarchiche
    parent_chunk_id: Optional[str] = Field(None, description="ID del chunk parent")
    child_chunk_ids: List[str] = Field(
        default_factory=list, description="ID dei chunk child"
    )

    # Immagini associate
    image_ids: List[str] = Field(
        default_factory=list, description="ID immagini in questo chunk"
    )
...
class DocumentChunk(BaseModel):
    ...
    sparse_embedding: Optional[Dict[str, float]] = Field(
        None, description="Embedding sparso"
    )
...
class ParameterRecord(BaseModel):
    ...
    type: str = Field(
        ..., description="Tipo del parametro (enum, string, number, etc.)"
    )
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Dipendenze da altri parametri"
    )
    related_errors: List[str] = Field(
        default_factory=list, description="Errori correlati"
    )
...
class SearchRequest(BaseModel):
    ...
    filters: Dict[str, Any] = Field(
        default_factory=dict, description="Filtri opzionali"
    )
    top_k: int = Field(default=10, description="Numero di risultati da restituire")
    include_metadata: bool = Field(
        default=True, description="Includere metadati nei risultati"
    )
...
class SearchResult(BaseModel):
    ...
    images: List[Dict[str, Any]] = Field(
        default_factory=list, description="Immagini associate"
    )


class ImageMetadata(BaseModel):
    """Metadati per immagini estratte dai documenti"""

    # Identificazione
    id: str = Field(..., description="ID univoco immagine (hash)")
    chunk_id: str = Field(default="", description="ID del chunk a cui appartiene")
    ...
    index_in_page: int = Field(
        default=0, description="Indice nell'array immagini pagina"
    )
    ...
    file_size_bytes: int = Field(..., description="Dimensione file in bytes")
    ...
    created_at: datetime = Field(
        default_factory=datetime.now, description="Data creazione"
    )
